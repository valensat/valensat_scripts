...
import pandas as pd
import os as os
DATA_CATALOG = pd.read_csv('L82021INFO.csv')
METEO = pd.read_csv('CBmeteoSEBS.csv')

DATA_CATALOG['year'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(6,10))
DATA_CATALOG['month'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(3,5))
DATA_CATALOG['day'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(0,2))
DATA_CATALOG['HOUR'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(11,12))

DATA_CATALOG['MINUTES'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(13,15))
...
for i in range(len(DATA4APPLICATION)):
    print(i)
...
import numpy as np
help(np.median)
print(set([out_folder]))
set(os.listdir()).isdisjoint(set([out_folder]))
help(pcraster.operations)
