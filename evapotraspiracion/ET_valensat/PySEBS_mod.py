...
DEM = readmap('./inputs_100/dem100.map')	# Digital Elevation Model [m]
nd = readmap('./inputs_100/NDVI.map') # NDVI map [-]
T = readmap('./inputs_100/LST.map') # Surface temperature [Kelvin]
albedo = readmap('./inputs_100/albedo.map') # Albedo map [-]
ems = readmap('./inputs_100/emiss.map') # emissivity [-]

# parameters

Trans = 0.788606 #transmissivity [0-1]
Lat = 40.34 #Latitude [dd]
DOY = 255 #Day of year
Time = 9.81 #Time of overpass [decimal hours]
z_pbl = 1000.0#PBL height [m]
alt_ms = 2 #Measurement height [m]
u_s = 0.6 #Wind speed [m/s]
t_s = 19.1 #Air temperature [Celcius]
p_s = 100000.0 #Air pressure [Pa]
hr_s = 0.41 #Relative humidity [0-1]
z_ms = alt_ms

# Define output files
lemap = './outprova/le.map'
hmap = './outprova/h.map'
gmap = './outprova/g0.map'
rnmap = './outprova/rn.map'
evaprmap = './outprova/evapr.map'
evapfrmap = './outprova/evapfr.map'
etmap = './outprova/et.map'
