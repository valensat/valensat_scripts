...
def mainSEBS(
    ...
    # Define output files
    lemap = './' + out_folder + '/le.map'
    hmap = './' + out_folder +'/h.map'
    gmap = './' + out_folder + '/g0.map'
    rnmap = './' + out_folder + '/rn.map'
    evaprmap = './' + out_folder + '/evapr.map'
    evapfrmap = './' + out_folder + '/evapfr.map'
    etmap = './' + out_folder + '/et.map'
...
import pandas as pd
import os as os
# read the Landsat 8 table
DATA_CATALOG = pd.read_csv('L82022INFO.csv')
# read the Meteo data table
METEO = pd.read_csv('CBmeteoSEBS2022.csv')
DATA_CATALOG['year'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(0,4))# based on the structure of the data catalog select the appropriate positions
DATA_CATALOG['month'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(5,7))# mind every time
...
#DATA4APPLICATION['pressure']= 88100
lst_folder = "ST"
ndvi_folder = "NDVI"
lai_folder = ""
fc_folder = ""
...
for i in range(0,len(DATA4APPLICATION)):
    ...
    print(filename)
    out_folder = "SEBS2_" + str(DATA4APPLICATION.iloc[i:i+1,:]['path'][i]) + str(DATA4APPLICATION.iloc[i:i+1,:]['row'][i]) + "_" + str(DATA4APPLICATION.iloc[i:i+1,:]['year'][i]) + str(DATA4APPLICATION.iloc[i:i+1,:]['month'][i]) + str(DATA4APPLICATION.iloc[i:i+1,:]['day'][i])
