...
out_folder = ""
# NDVI must be already between 0 and 1, as the postptocessing of NDVI was suppressed to avoid problems.
...
def mainSEBS(
        ...
        out_folder,
        id_suffix,
        Trans, # transmissivity (from 0 to 1)
        Lat, # latitude in decimal degrees, for radiation computation
        DOY, # julian day of the year for radiation calculation
        Time, #Time of overpass [decimal hours]
        alt_ms, #Measurement height [m]
        u_s, #Wind speed [m/s]
        t_s, #Air temperature [Celcius]
        p_s, #Air pressure [Pa]
        hr_s, #Relative humidity [0-1]
        z_ms): # heightt of measurment, for simplicity we assume it is the same as alt_ms
    ...
    def median(map):
       ...
       if not (Median[0] > 0):
           Minimum = cellvalue(mapminimum(map), 0, 0)
           Maximum = cellvalue(mapmaximum(map), 0, 0)
           Median = ((Maximum[0]+Minimum[0])/2, True)
           print("median replaced to half range")
           if not (Median[0] > 0):
               Median = (0.1, True)
    ...
    DEM = readmap('./inputs_100/DEM30.map')	# Digital Elevation Model [m]
    nd = readmap('./' + ndvi_folder + "/" + filename) # NDVI map [-]
    T = readmap('./' + lst_folder + "/" + filename) # Surface temperature [Kelvin]
    albedo = readmap('./' + albedo_folder + "/" + filename) # Albedo map [-]
    ems = readmap('./' + emiss_folder + "/" + filename) # emissivity [-]
    
    pcraster.matplotlib.plot(DEM, labels=None, title="DEM")
    pcraster.matplotlib.plot(nd, labels=None, title="NDVI")
    pcraster.matplotlib.plot(T, labels=None, title="LST")
    pcraster.matplotlib.plot(ems, labels=None, title="emissivity")
    pcraster.matplotlib.plot(albedo, labels=None, title="albedo")
    ...
    # Define output files
    lemap = './' + out_folder + '/le' + id_suffix + '.map'
    hmap = './' + out_folder +'/h' + id_suffix + '.map'
    gmap = './' + out_folder + '/g0' + id_suffix + '.map'
    rnmap = './' + out_folder + '/rn' + id_suffix + '.map'
    evaprmap = './' + out_folder + '/evapr' + id_suffix + '.map'
    evapfrmap = './' + out_folder + '/evapfr' + id_suffix + '.map'
    etmap = './' + out_folder + '/et' + id_suffix + '.map'
    
    print("Initializing SEBS.",)
    # Initialize model starttime for calculation runtime
    starttime = time()
    # Check input data
    #WB = pcrlt(nd,0.0) # WATER BODIES
    #TH = pcrgt(nd,1.0) #TOO HIGH  NDVI VALUES
    #nd = ifthenelse(pcror(WB,TH), 1.0, nd) # Convert waterbodies to 1.0 --> soilflux is minimal
...
# GET THE METADATA IN A NICE TABLE, TO BE ABLE TO LOOP ON IT 
import pandas as pd
import os as os
DATA_CATALOG = pd.read_csv('L82022INFO.csv')
METEO = pd.read_csv('CBmeteoSEBS2022.csv')
print('cominciamo')
DATA_CATALOG['year'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(0,4))
print('ok')
DATA_CATALOG['month'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(5,7))
print('ok')
DATA_CATALOG['day'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(8,10))
print('ok')
DATA_CATALOG['HOUR'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(11,13))
print('ok')
DATA_CATALOG['MINUTES'] = pd.to_numeric(DATA_CATALOG['time'].str.slice(14,16))
print('ok')
DATA_CATALOG['DECHOUR'] = DATA_CATALOG['HOUR'] + (DATA_CATALOG['MINUTES']/60)+1
DATA_CATALOG['RHOUR'] = round(DATA_CATALOG['DECHOUR'])
DATA_CATALOG['ALTITUDE'] = 1160
DATA_CATALOG['LATITUDE'] = 41.41
DATA_CATALOG['datetime'] = pd.to_datetime(DATA_CATALOG[['year', 'month', 'day']])
DATA_CATALOG['julian'] = 1 + pd.DatetimeIndex(DATA_CATALOG['datetime']).to_julian_date()- pd.DatetimeIndex(pd.to_datetime(pd.DataFrame({'year': [2021],
                                                                                                     'month' : [1],
                                                                                                     'day' : [1]}))).to_julian_date()

DATA4APPLICATION = pd.merge(DATA_CATALOG, METEO, 
                            left_on=['year', 'month', 'day', 'RHOUR'],
                            right_on = ['year', 'month', 'day', 'hour'], 
                            how = 'left')

print(DATA4APPLICATION.iloc[0:1,:]['pressure'][0])
#declare the variables for selecting the image

lst_folder = "ST"
ndvi_folder = "NDVI"
lai_folder = ""
fc_folder = ""
albedo_folder = "ALBEDO"
emiss_folder = "EMISS"
dem_path = './inputs_100/DEM30.map'
filename = ""

for i in range(0,len(DATA4APPLICATION)):
    print(i)
    Trans = 0.788606 #transmissivity [0-1]
    Lat = 41.41 
    DOY = DATA4APPLICATION.iloc[i:i+1, :]['doy'][i] #Day of year
    Time = DATA4APPLICATION.iloc[i:i+1, :]['DECHOUR'][i] #Time of overpass [decimal hours]

    alt_ms = DATA4APPLICATION.iloc[i:i+1,:]['ALTITUDE'][i] #Measurement height [m]
    u_s = DATA4APPLICATION.iloc[i:i+1,:]['Wind'][i] #Wind speed [m/s]
    t_s = DATA4APPLICATION.iloc[i:i+1,:]['AirT'][i] #Air temperature [Celcius]
    p_s = DATA4APPLICATION.iloc[i:i+1,:]['pressure'][i] #Air pressure [Pa]
    hr_s = DATA4APPLICATION.iloc[i:i+1,:]['RHp'][i] #Relative humidity [0-1]
    z_ms = alt_ms
    dem_path = './inputs_100/DEM30.map'
    filename = 'L' + DATA4APPLICATION.iloc[i:i+1,:]['name'][i]+".map"
    print(filename)
    out_folder = "SEBS_2022_new" #+ str(DATA4APPLICATION.iloc[i:i+1,:]['path'][i]).rjust(3, '0') + str(DATA4APPLICATION.iloc[i:i+1,:]['row'][i]).rjust(3, '0') + "_" + str(DATA4APPLICATION.iloc[i:i+1,:]['year'][i]) + str(DATA4APPLICATION.iloc[i:i+1,:]['month'][i]).rjust(2, '0') + str(DATA4APPLICATION.iloc[i:i+1,:]['day'][i]).rjust(2, '0')
    id_suffix = "_"  + str(DATA4APPLICATION.iloc[i:i+1,:]['path'][i]).rjust(3, '0') + str(DATA4APPLICATION.iloc[i:i+1,:]['row'][i]).rjust(3, '0') + "_" + str(DATA4APPLICATION.iloc[i:i+1,:]['year'][i]) + str(DATA4APPLICATION.iloc[i:i+1,:]['month'][i]).rjust(2, '0') + str(DATA4APPLICATION.iloc[i:i+1,:]['day'][i]).rjust(2, '0')
    print(out_folder)
    print(id_suffix)
    try: 
        if(set(os.listdir()).isdisjoint(set([out_folder]))):
            os.mkdir(out_folder)
        mainSEBS(
                dem_path = dem_path,
                filename = filename,
                ...
                Lat = 41.41 ,
                DOY = DATA4APPLICATION.iloc[i:i+1, :]['doy'][i], #Day of year
                Time = float(DATA4APPLICATION.iloc[i:i+1, :]['DECHOUR'][i]), #Time of overpass [decimal hours]
                ...
                p_s = DATA4APPLICATION.iloc[i:i+1,:]['pressure'][i], #Air pressure [Pa]
                hr_s = DATA4APPLICATION.iloc[i:i+1,:]['RHp'][i], #Relative humidity [0-1]
                z_ms = DATA4APPLICATION.iloc[i:i+1,:]['ALTITUDE'][i])
    except:
        print('SEBS failed')
